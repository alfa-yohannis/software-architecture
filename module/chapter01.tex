...
\begin{enumerate}
  ...
  \item Mengukur waktu bolak-balik satu \textit{request} dan perubahannya ketika
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{|l|X|X|}
    ...
    \hline
    Kemudahan perawatan & Berapa banyak berkas berubah untuk satu \textit{change request}? & Jumlah berkas dan baris \\
    \hline
    Kemampuan skala & Apa yang terjadi ketika jumlah pengguna naik sepuluh kali? & Latensi pada beban bertingkat \\
    \hline
    Kinerja & Berapa lama satu \textit{request} dilayani? & Median dan persentil ke-95 \\
    \hline
...
Daftar aspek tersebut bukan satu-satunya yang dipakai. Microsoft menyusun
daftar serupa untuk membandingkan gaya arsitektur pada panduan
resminya~\cite{microsoft2024webapps}.
...
Pemisahan peminta dan pelayan tersebut menjadi dasar hampir seluruh gaya
arsitektur berikutnya. Fielding~\cite{fielding2000architectural} menelusuri
pembatasan yang melekat padanya, termasuk keharusan setiap \textit{request} menempuh
jaringan sehingga biayanya tidak dapat dihapus.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[
    ...
    \draw[panah] (k1.east) -- node[above, font=\tiny, pos=0.55] {\textit{request}} (srv.west);
    \draw[panah] (k2.east) -- (srv.west);
    \draw[panah] (k3.east) -- (srv.west);
  \end{tikzpicture}
  \caption{Banyak klien memanggil satu server. Seluruh \textit{request} berawal dari
...
Server membaca parameter \textit{request}, menghitung, lalu membalas dalam bentuk
JavaScript Object Notation (JSON).

\begin{lstlisting}[style=PythonStyle, caption={Penanganan satu \textit{request} pada
...
\textit{request}, lalu menampilkan jawaban. Tidak ada tabel kurs dan tidak ada
...
\textbf{Biaya perjalanan.} Setiap \textit{request} harus menempuh jaringan dua kali,
...
\textbf{Biaya antrean.} Satu server melayani banyak klien. Ketika \textit{request}
datang lebih cepat daripada kemampuan server melayaninya, sisanya menunggu.
Server pada contoh bab ini melayani satu \textit{request} pada satu waktu, sehingga
...
seribu milidetik. Sebagian \textit{request} karena itu menunggu sangat lama, sementara
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{|X|X|}
    ...
    \hline
    Aturan bisnis tidak dapat diubah dari sisi klien & Setiap \textit{request} menanggung biaya perjalanan jaringan \\
    \hline
...
Lingkungan kerja disiapkan sekali saja, sebelum latihan pertama dijalankan.
Skrip pada Listing~\ref{lst:venv-ch01} membuat \textit{virtual environment}
(venv) di \berkas{.venv}, lalu memasang pustaka yang tercantum di
\berkas{sources/requirements.txt}. Venv memisahkan pustaka latihan dari Python
bawaan sistem, sehingga percobaan pada satu bab tidak mengganggu bab lainnya.
Perintahnya dijalankan dari akar repositori. Pada terminal baru, cukup baris
pengaktifannya yang diulang, sedangkan pembuatannya tidak perlu diulang. Bagi
pengguna Windows, baris pengaktifan tersebut diganti dengan
\berkas{.venv\Scripts\activate}.

\begin{lstlisting}[language=bash, caption={Menyiapkan venv sekali sebelum
  seluruh latihan, skrip tersedia di \berkas{sources/siapkan_venv.sh}},
  label=lst:venv-ch01]
bash sources/siapkan_venv.sh
source .venv/bin/activate
python --version
# Keluarannya: Python 3.14.7
\end{lstlisting}
...
Venv diaktifkan pada kedua terminal tersebut.
...
Latihan ini mengukur waktu bolak-balik satu \textit{request}. Langkah kerjanya sebagai
berikut.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{|X|l|l|l|}
    ...
    \hline
    Ke-1, 200 \textit{request} & 0,237 ms & 0,351 ms & 7,391 ms \\
    \hline
  \end{tabularx}
\end{table}
...
terbesar pada pengukuran pertama jauh melebihi mediannya. Jelaskan sebabnya. Kedua, jelaskan bagian mana dari waktu tersebut yang tetap ada
...
tumbuh perlahan sedangkan nilai terbesar melonjak. Jelaskan apa yang dialami
sebagian kecil \textit{request} tersebut. Kedua, jelaskan mengapa gejala itu
