...
class Perintah(Protocol):
  def __call__(self, argumen: list[str]) -> str:
    # Menjalankan perintah lalu mengembalikan teks hasilnya
    ...
\end{lstlisting}
  \vspace{4pt}
  \begin{itemize}\small
...
\begin{frame}{{\Large Diagram Kelas Inti dan \textit{Plugin}}}
  ...
  \begin{columns}[c]
    \begin{column}{0.52\textwidth}
      \gambarkolom{\includegraphics{kelas-microkernel.pdf}}
    \end{column}
...
\begin{frame}{{\Large Analogi: Stopkontak di Dinding}}
  \vspace{10pt}
  \begin{itemize}\small
    \item Dinding menyediakan lubang dengan bentuk yang sudah disepakati.
    \item Pemasang dinding tidak perlu tahu perangkat apa yang kelak
          ditancapkan.
    \item Proyektor, kipas, dan pengisi daya sama-sama masuk selama bentuk
          stekernya sesuai.
    \item Dinding tidak ikut rusak ketika satu perangkat terbakar, sebab
          pengaman memutus arus ke lubang itu saja.
  \end{itemize}

  \vspace{10pt}
  {\small Bentuk lubang itulah kontraknya, dan pengaman itulah pembungkus
  galatnya.}
\end{frame}
...
\begin{frame}{{\Large Hasil Pengukuran}}
  ...
  \begin{tabularx}{0.9\textwidth}{|X|l|}
    ...
    \hline
    Median aktivasi lambat & 0,6785 ms \\
    \hline
    Median aktivasi awal & 2,1741 ms \\
    \hline
    Selisih median antar jalur & 1,4956 ms \\
    \hline
    Aktivasi awal lebih lambat & 3,2 kali \\
    \hline
    \textit{Range} dibagi selisih & 0,7 kali \\
    \hline
...
\end{lstlisting}
  ...
  \begin{enumerate}\small
    \item Catat baris \texttt{Plugin disebut inti} beserta ketiga angka di
          bawahnya.
    \item Salin \texttt{plugin\_riwayat.py} beserta manifesnya menjadi
          \texttt{plugin\_pajak.py}, lalu ubah nama dan perintahnya.
...
\end{lstlisting}
  ...
  \vspace{3pt}
  \begin{enumerate}\small
    \item Tunjukkan baris pada \texttt{plugin\_rusak.py} yang menyebabkan
          kegagalannya.
